...
def train(args):
    ...
    if args.t_test_path is not None:
        dsets["test"] = data_list.ImageList(open(args.t_test_path).readlines(), transform=image_test())
    else:
    ...
    if args.dset == 'imagenet_caltech':
        if args.s == 0:
            ad_nets = [network.AdversarialNetwork(base_network.output_num(), 128, args.max_iterations).cuda() for _ in range(args.class_num)]
        else:
            ad_nets = [network.AdversarialNetwork(base_network.output_num(), 256, args.max_iterations).cuda() for _ in range(args.class_num)]
    else:
        ad_nets = [network.AdversarialNetwork(base_network.output_num(), 1024, args.max_iterations).cuda() for _ in range(args.class_num)]
    parameter_list = base_network.get_parameters()
    for ad_net in ad_nets:
        parameter_list = parameter_list + ad_net.get_parameters()
    ...
    for i in range(args.max_iterations + 1):
        ...
        if args.da_loss == 'dann':
            transfer_loss = my_loss.DANN_multi(features, ad_nets, class_weight, softmax_out.detach(), entropy, args.san_weight*network.calc_coeff(i, 1, 0, 10, args.max_iterations)/args.class_num, cls_weight, len_share)       
        else:
            transfer_loss = my_loss.CDAN_multi(features, ad_nets, class_weight, softmax_out.detach(), entropy, args.san_weight*network.calc_coeff(i, 1, 0, 10, args.max_iterations)/args.class_num, cls_weight, len_share)

        softmax_tar_out = torch.nn.Softmax(dim=1)(outputs_target)
        if args.self_training == 'pseudo':
            confidence, pseudo_label = torch.max(softmax_tar_out.detach(), dim=1)
            self_training_weight = (confidence > args.conf_thres).float()
            tar_loss = torch.sum(self_training_weight * nn.CrossEntropyLoss(reduction='none')(outputs_target, pseudo_label)) / (1e-8+torch.sum(self_training_weight))
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--weight_cls', action='store_false')
    parser.add_argument('--alpha', type=float, default=1)
    parser.add_argument('--self_training', default='pseudo')
    parser.add_argument('--da_loss', default='dann')
    parser.add_argument('--conf_thres', default=0.9, type=float)
    parser.add_argument('--san_weight', default=1., type=float)

    args = parser.parse_args()
    os.environ["CUDA_VISIBLE_DEVICES"] = args.gpu_id

    args.t_test_path = None
    ...
    if args.dset == 'openmic':
        names = ['source', 'target_train_1', 'target_train_2', 'target_train_3', 'target_train_4', 'target_train_5',
                           'target_test_1', 'target_test_2', 'target_test_3', 'target_test_4', 'target_test_5']
        k = 5
        args.class_num = 10
        args.t_test_path = './data/' + args.dset + '/' + names[args.t+5] + '_' + str(k) + '_list.txt'
        args.max_iterations = 6000
        args.test_interval = 500
        args.lr=1e-3

    if args.dset == 'visda-super1':
        names = ['synthetic_super', 'synthetic_super_half', 'real_super_2', 'real_super_3', 'real_super_4', 'real_super_5']
        k = 'no'
        args.class_num = 6
        args.max_iterations = 6000
        args.test_interval = 500
